...
#if 0
static const int row_gpio[ROW_NR] = {10,11,12};
static const int col_gpio[COL_NR] = {14,15,16};
#else
...
static void matrix_kp_get_col_status(unsigned char * status, int cnt)
{
	...
	{
		printk("\n before col scan,status[%d] = 0x%x\n",row,status[row]);
		//ret = gpio_request(row_gpio[row],"DaVinci");
		//printk("\n gpio_request = %d\n",ret);
		ret = gpio_direction_output(row_gpio[row], 0);		
		printk("\n gpio_direction_output[%d] = %d\n",row,ret);
		...
		{		   
		   ...
		   //val = gpio_get_value(row_gpio[col]);
		   if (val == 0)
		   {
			   status[row] |= (1<<col);
		   }
		...
		//ret = gpio_direction_input(col_gpio[row]);
		printk("\n gpio_direction_input[%d]=%d\n",row,ret);
		if (col_scan_udelay > 0)
		{
								...
								int key_status,struct input_dev * pdev)
{
	...
	{		
		changed = col_status[rowindex] ^ status[rowindex];
        for (col = 1; col < COL_NR; col++)
		{
		   if (changed & (1<<col))
		   {	
		      nowcol = col;
								 ...
								 struct input_dev * pdev)
{
	...
	for (row = 0; row < (cnt-1); row++)
	{
		...
		printk(KERN_ERR "\n1 col_status[%d]=0x%x,new_status[%d]=0x%x\n",row,col_status[row],row,status[row]);
		if (!changed)
		{
			 continue;
		}
	...
	row = cnt - 1;
	changed = col_status[row] ^ status[row];
	printk(KERN_ERR "\n2 status[%d]=0x%x,col_status[%d]=0x%x,changed=%d\n",row,status[row],row,col_status[row],changed);
	if(changed)
	{
		process_flage = 1;		
		if(status[row] == 0)
...
#else
	matrix_kp_get_col_status(new_status, ROW_NR);
	changed = matrix_kp_changed(new_status,ROW_NR,col_status);
	if(changed)
	{
	  tdealy = jiffies + (20*HZ/1000);
	  while(jiffies < tdealy)
...
static int matrix_kp_open(struct input_dev *dev)
{
	...
	ret = gpio_request(12,"DaVinci");
	printk(KERN_ERR "\ngpio_request(12),ret=%d\n",ret);
