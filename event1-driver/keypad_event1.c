...
#else
static const char row_gpio[ROW_NR] = {10};
	...
	if(changed)
	{
	  for (col = 0; col < COL_NR; col++)
	  {
		 if (changed & (1<<col))
		 {
			...
			process_flage = 1;
            printk(KERN_ERR "\nkey_matrix=%d,row=%d,col=%d\n",key_matrix[row][col],row,col);
			input_sync(pdev);
...
static void matrix_kp_scan(unsigned long data)
{
	...
	//changed = matrix_kp_changed(temp_status,ROW_NR-1,col_status); 150909 comment
